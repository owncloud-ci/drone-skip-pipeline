...
def linux(ctx, arch):
    if ctx.build.event == "tag":
        build = [
            'go build -v -ldflags "-X main.version=%s" -a -tags netgo -o release/linux/%s/drone-skip-pipeline ./cmd/drone-skip-pipeline' % (ctx.build.ref.replace("refs/tags/v", ""), arch),
        ]
    else:
        build = [
            'go build -v -ldflags "-X main.version=%s" -a -tags netgo -o release/linux/%s/drone-skip-pipeline ./cmd/drone-skip-pipeline' % (ctx.build.commit[0:8], arch),
        ]
    ...
    steps = [
        ...
        },
        {
            ...
            "image": "owncloudci/golang:1.18",
            "commands": [
                "./release/linux/%s/drone-skip-pipeline --help" % (arch),
            ],
        },
    ]
    ...
    "steps": [
        {
            ...
            "environment": {
                ...
                "PUSHRM_SHORT": "Drone plugin to skip pipelines based on changed files",
